create or replace function badminton_match.bm_publish_session(p_id text, p_snapshot jsonb)
returns void
language plpgsql
security definer
...
create or replace function badminton_match.bm_get_session(p_id text)
returns jsonb
language sql
security definer
set search_path = public, badminton_match
...
create or replace function badminton_match.bm_list_sessions()
returns table (
  id text,
  title text,
  date text,
...
create or replace function badminton_match.bm_list_players()
returns table (
  name text,
  gender text,
  tier integer
...
create or replace function badminton_match.bm_get_player_stats(p_name text)
returns jsonb
language plpgsql
security definer
set search_path = public, badminton_match
...
create or replace function badminton_match.bm_publish_tournament(p_id text, p_snapshot jsonb)
returns void
language plpgsql
security definer
set search_path = public, badminton_match
...
create or replace function badminton_match.bm_get_tournament(p_id text)
returns jsonb
language sql
security definer
set search_path = public, badminton_match
...
grant usage on schema badminton_match to anon, authenticated, service_role;
grant execute on function badminton_match.bm_publish_session(text, jsonb) to anon, authenticated, service_role;
grant execute on function badminton_match.bm_get_session(text) to anon, authenticated, service_role;
grant execute on function badminton_match.bm_list_sessions() to anon, authenticated, service_role;
grant execute on function badminton_match.bm_list_players() to anon, authenticated, service_role;
grant execute on function badminton_match.bm_get_player_stats(text) to anon, authenticated, service_role;
grant execute on function badminton_match.bm_publish_tournament(text, jsonb) to anon, authenticated, service_role;
grant execute on function badminton_match.bm_get_tournament(text) to anon, authenticated, service_role;

drop function if exists public.bm_publish_session(text, jsonb);
drop function if exists public.bm_get_session(text);
drop function if exists public.bm_list_sessions();
drop function if exists public.bm_list_players();
drop function if exists public.bm_get_player_stats(text);
drop function if exists public.bm_publish_tournament(text, jsonb);
drop function if exists public.bm_get_tournament(text);
