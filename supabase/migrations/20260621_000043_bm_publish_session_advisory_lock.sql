-- Add advisory lock to publish_session (parity with publish_tournament)
--
-- publish_tournament already uses pg_try_advisory_xact_lock for global
-- mutual exclusion before the row-level FOR UPDATE NOWAIT. publish_session
-- only uses FOR UPDATE NOWAIT, which doesn't protect the INSERT path for
-- new sessions (no row to lock). Two concurrent requests for the same new
-- session ID can both pass the row lock and one will get a raw PostgreSQL
-- unique constraint violation instead of the friendly error message.
--
-- Fix: add pg_try_advisory_xact_lock before the SELECT...INTO, matching
...
begin
  ...
  -- Protects the INSERT path for new sessions where FOR UPDATE NOWAIT
  -- cannot lock a non-existent row.
