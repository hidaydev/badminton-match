-- Fix: use status column for lock check instead of non-existent snapshot column
--
-- The bm.sessions table has a status column with allowed values:
-- 'draft', 'locked', 'published', 'completed', 'archived'
-- We use status='locked' to check if a session is locked.
...
begin
  ...
  if v_session_found then
    ...
    -- Lock enforcement: reject writes when existing session is locked
...
-- Fix unlock_session to use status column instead of snapshot
