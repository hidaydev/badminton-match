-- Fix publish_session and unlock_session RPCs
--
-- H1: publish_session lock check now rejects any non-draft status
--     (previously only checked status='locked', missing completed/archived).
-- M3: Reorder publish_session — acquire advisory lock + check lock status
--     BEFORE running the expensive validate_session_snapshot.
-- M4: unlock_session now bumps version when resetting to draft.
