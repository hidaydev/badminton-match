-- Fix publish_session: found flag overwritten by validation CTEs
--
-- Root cause: PL/pgSQL's `found` boolean is reset by every SELECT...INTO.
-- The session lookup sets found correctly, but subsequent validation CTEs
-- (unresolved players, invalid refs, duplicate canonical) each do their own
-- SELECT...INTO which overwrites `found`. When all players are resolved,
-- the last CTE returns rows → found=true → code takes UPDATE path for a
-- session that doesn't exist → session never inserted → session_courts
-- trigger fails because the session row is missing.
--
-- This also fixes the v_id/v_share_id NULL issue from migration 000041
-- (SELECT...INTO sets all targets to NULL when no row found).
--
-- Fix: capture found into a local boolean immediately after the session
-- lookup, before any CTE can overwrite it.
...
-- Same fix for publish_tournament: save found before CTEs overwrite it
