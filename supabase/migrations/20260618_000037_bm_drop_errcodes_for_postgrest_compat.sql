-- Scope 8 revert: Remove USING errcode clauses from publish_session and publish_tournament
--
-- Migration 000036 restored SQLSTATE errcodes (40001, 55P03) on concurrency
-- exceptions. Testing revealed that PostgREST (via Supabase REST API) hangs
-- when a plpgsql function raises an exception with USING errcode on the
-- version-mismatch path. This was the same issue migration 000009 fixed
-- by removing the errcodes in the first place.
--
-- This migration re-applies both functions identically to 000036 but without
-- the USING errcode clauses. Client-side errcode matching in errors.ts
-- remains as harmless fallback; primary matching falls back to message text.
--
-- When Supabase upgrades PostgREST and the hang is resolved, re-introduce
-- the errcode clauses via a new migration.
