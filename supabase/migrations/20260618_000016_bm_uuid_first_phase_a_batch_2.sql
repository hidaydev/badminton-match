...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bm_sessions_id_internal_id_key'
      and conrelid = 'bm.sessions'::regclass
  ) then
    alter table bm.sessions
      add constraint bm_sessions_id_internal_id_key unique (id, internal_id);
  end if;
end
$$;

alter table bm.session_courts
  add column if not exists session_internal_id uuid;

update bm.session_courts sc
set session_internal_id = s.internal_id
from bm.sessions s
where s.id = sc.session_id
  and sc.session_internal_id is null;

alter table bm.session_courts
  alter column session_internal_id set not null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bm_session_courts_session_identity_fk'
      and conrelid = 'bm.session_courts'::regclass
  ) then
    alter table bm.session_courts
      add constraint bm_session_courts_session_identity_fk
...
update bm.session_players sp
set session_internal_id = s.internal_id
from bm.sessions s
where s.id = sp.session_id
  and sp.session_internal_id is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bm_session_players_session_identity_fk'
      and conrelid = 'bm.session_players'::regclass
  ) then
    alter table bm.session_players
      add constraint bm_session_players_session_identity_fk
      foreign key (session_id, session_internal_id)
      references bm.sessions (id, internal_id)
      on delete cascade;
  end if;
...
update bm.fix_matches fm
set session_internal_id = s.internal_id
from bm.sessions s
where s.id = fm.session_id
  and fm.session_internal_id is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bm_fix_matches_session_identity_fk'
      and conrelid = 'bm.fix_matches'::regclass
  ) then
    alter table bm.fix_matches
      add constraint bm_fix_matches_session_identity_fk
      foreign key (session_id, session_internal_id)
      references bm.sessions (id, internal_id)
      on delete cascade;
  end if;
...
update bm.scheduled_games sg
set session_internal_id = s.internal_id
from bm.sessions s
where s.id = sg.session_id
  and sg.session_internal_id is null;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'bm_scheduled_games_session_identity_fk'
      and conrelid = 'bm.scheduled_games'::regclass
  ) then
    alter table bm.scheduled_games
      add constraint bm_scheduled_games_session_identity_fk
      foreign key (session_id, session_internal_id)
      references bm.sessions (id, internal_id)
      on delete cascade;
  end if;
