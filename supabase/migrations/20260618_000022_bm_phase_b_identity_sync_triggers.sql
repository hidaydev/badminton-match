create or replace function bm.sync_session_identity()
returns trigger
language plpgsql
...
declare
  v_session_id text;
  v_session_internal_id uuid;
begin
  if new.session_id is null and new.session_internal_id is null then
    raise exception 'session identity is required for %', tg_table_name;
  end if;

  if new.session_internal_id is null then
    select s.id, s.internal_id
    into v_session_id, v_session_internal_id
    from bm.sessions s
    where s.id = new.session_id;
  elsif new.session_id is null then
    select s.id, s.internal_id
    into v_session_id, v_session_internal_id
    from bm.sessions s
    where s.internal_id = new.session_internal_id;
  else
    select s.id, s.internal_id
    into v_session_id, v_session_internal_id
    from bm.sessions s
    where s.id = new.session_id
      and s.internal_id = new.session_internal_id;
  end if;

  if v_session_internal_id is null then
    raise exception 'invalid session identity for %', tg_table_name;
  end if;

  new.session_id := v_session_id;
  new.session_internal_id := v_session_internal_id;
  return new;
end;
$$;

create or replace function bm.sync_fix_match_slot_identities()
returns trigger
language plpgsql
set search_path = bm, public
as $$
declare
  v_fix_match_id bigint;
  v_fix_match_internal_id uuid;
  v_session_player_id bigint;
  v_session_player_internal_id uuid;
begin
  if new.fix_match_id is null and new.fix_match_internal_id is null then
    raise exception 'fix match identity is required for %', tg_table_name;
  end if;

  if new.fix_match_internal_id is null then
    select fm.id, fm.internal_id
    into v_fix_match_id, v_fix_match_internal_id
    from bm.fix_matches fm
    where fm.id = new.fix_match_id;
  elsif new.fix_match_id is null then
    select fm.id, fm.internal_id
    into v_fix_match_id, v_fix_match_internal_id
    from bm.fix_matches fm
    where fm.internal_id = new.fix_match_internal_id;
  else
    select fm.id, fm.internal_id
    into v_fix_match_id, v_fix_match_internal_id
    from bm.fix_matches fm
    where fm.id = new.fix_match_id
      and fm.internal_id = new.fix_match_internal_id;
  end if;

  if v_fix_match_internal_id is null then
    raise exception 'invalid fix match identity for %', tg_table_name;
  end if;

  new.fix_match_id := v_fix_match_id;
  new.fix_match_internal_id := v_fix_match_internal_id;

  if new.session_player_id is null and new.session_player_internal_id is null then
    return new;
  end if;

  if new.session_player_internal_id is null then
    select sp.id, sp.internal_id
    into v_session_player_id, v_session_player_internal_id
    from bm.session_players sp
    where sp.id = new.session_player_id;
  elsif new.session_player_id is null then
    select sp.id, sp.internal_id
    into v_session_player_id, v_session_player_internal_id
    from bm.session_players sp
    where sp.internal_id = new.session_player_internal_id;
  else
    select sp.id, sp.internal_id
    into v_session_player_id, v_session_player_internal_id
    from bm.session_players sp
    where sp.id = new.session_player_id
      and sp.internal_id = new.session_player_internal_id;
  end if;

  if v_session_player_internal_id is null then
    raise exception 'invalid session player identity for %', tg_table_name;
  end if;

  new.session_player_id := v_session_player_id;
  new.session_player_internal_id := v_session_player_internal_id;
  return new;
end;
$$;

create or replace function bm.sync_scheduled_game_player_identities()
returns trigger
language plpgsql
set search_path = bm, public
as $$
declare
  v_scheduled_game_id bigint;
  v_scheduled_game_internal_id uuid;
  v_session_player_id bigint;
  v_session_player_internal_id uuid;
begin
  if new.scheduled_game_id is null and new.scheduled_game_internal_id is null then
    raise exception 'scheduled game identity is required for %', tg_table_name;
  end if;

  if new.scheduled_game_internal_id is null then
    select sg.id, sg.internal_id
    into v_scheduled_game_id, v_scheduled_game_internal_id
    from bm.scheduled_games sg
    where sg.id = new.scheduled_game_id;
  elsif new.scheduled_game_id is null then
    select sg.id, sg.internal_id
    into v_scheduled_game_id, v_scheduled_game_internal_id
    from bm.scheduled_games sg
    where sg.internal_id = new.scheduled_game_internal_id;
  else
    select sg.id, sg.internal_id
    into v_scheduled_game_id, v_scheduled_game_internal_id
    from bm.scheduled_games sg
    where sg.id = new.scheduled_game_id
      and sg.internal_id = new.scheduled_game_internal_id;
  end if;

  if v_scheduled_game_internal_id is null then
    raise exception 'invalid scheduled game identity for %', tg_table_name;
  end if;

  new.scheduled_game_id := v_scheduled_game_id;
  new.scheduled_game_internal_id := v_scheduled_game_internal_id;

  if new.session_player_id is null and new.session_player_internal_id is null then
    raise exception 'session player identity is required for %', tg_table_name;
  end if;
...
create or replace function bm.sync_scheduled_game_identity()
returns trigger
language plpgsql
set search_path = bm, public
as $$
...
drop trigger if exists bm_session_courts_sync_session_identity on bm.session_courts;
create trigger bm_session_courts_sync_session_identity
before insert or update on bm.session_courts
for each row
execute function bm.sync_session_identity();

drop trigger if exists bm_session_players_sync_session_identity on bm.session_players;
create trigger bm_session_players_sync_session_identity
before insert or update on bm.session_players
for each row
execute function bm.sync_session_identity();

drop trigger if exists bm_fix_matches_sync_session_identity on bm.fix_matches;
create trigger bm_fix_matches_sync_session_identity
before insert or update on bm.fix_matches
for each row
execute function bm.sync_session_identity();

drop trigger if exists bm_scheduled_games_sync_session_identity on bm.scheduled_games;
create trigger bm_scheduled_games_sync_session_identity
before insert or update on bm.scheduled_games
for each row
execute function bm.sync_session_identity();

drop trigger if exists bm_fix_match_slots_sync_identities on bm.fix_match_slots;
create trigger bm_fix_match_slots_sync_identities
before insert or update on bm.fix_match_slots
for each row
execute function bm.sync_fix_match_slot_identities();

drop trigger if exists bm_scheduled_game_players_sync_identities on bm.scheduled_game_players;
create trigger bm_scheduled_game_players_sync_identities
before insert or update on bm.scheduled_game_players
for each row
execute function bm.sync_scheduled_game_player_identities();

drop trigger if exists bm_game_progress_sync_scheduled_game_identity on bm.game_progress;
create trigger bm_game_progress_sync_scheduled_game_identity
before insert or update on bm.game_progress
for each row
execute function bm.sync_scheduled_game_identity();

drop trigger if exists bm_game_scores_sync_scheduled_game_identity on bm.game_scores;
create trigger bm_game_scores_sync_scheduled_game_identity
before insert or update on bm.game_scores
for each row
execute function bm.sync_scheduled_game_identity();
