...
func (s *TournamentStore) Load(ctx context.Context, id string) (*domain.TournamentSnapshot, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT t.id::text FROM `+s.schema+`.tournaments t
	...
	if err := s.pool.QueryRow(ctx, `
		SELECT name, event_date::text, version FROM `+s.schema+`.tournaments WHERE id = $1::uuid`,
	...
	rows, err := s.pool.Query(ctx, `
		SELECT tp.seed, tp.pair_name FROM `+s.schema+`.tournament_pairs tp
	...
	rows, err = s.pool.Query(ctx, `
		...
		FROM `+s.schema+`.tournament_groups tg
		JOIN `+s.schema+`.tournament_pairs tp ON tp.id = tg.pair_id
	...
	rows, err = s.pool.Query(ctx, `
		...
		LEFT JOIN `+s.schema+`.tournament_pairs tp_a ON tp_a.id = tm.pair_a_id
		LEFT JOIN `+s.schema+`.tournament_pairs tp_b ON tp_b.id = tm.pair_b_id
		LEFT JOIN `+s.schema+`.players pl ON pl.id = tm.pic_player_id
...
func (s *TournamentStore) Save(ctx context.Context, id string, snap *domain.TournamentSnapshot) (*domain.TournamentSnapshot, error) {
	...
	if _, err := tx.Exec(ctx, `DELETE FROM `+s.schema+`.tournament_matches WHERE tournament_id = $1::uuid`, rowID); err != nil {
		return nil, err
	}
	if _, err := tx.Exec(ctx, `DELETE FROM `+s.schema+`.tournament_groups WHERE tournament_id = $1::uuid`, rowID); err != nil {
		return nil, err
	}
	if _, err := tx.Exec(ctx, `
		DELETE FROM `+s.schema+`.tournament_pair_players WHERE pair_id IN (SELECT id FROM `+s.schema+`.tournament_pairs WHERE tournament_id = $1::uuid)`,
		rowID); err != nil {
		return nil, err
	}
	if _, err := tx.Exec(ctx, `DELETE FROM `+s.schema+`.tournament_pairs WHERE tournament_id = $1::uuid`, rowID); err != nil {
		return nil, err
	}
	...
	for i, p := range snap.Pairs {
		...
		if err := tx.QueryRow(ctx, `
			INSERT INTO `+s.schema+`.tournament_pairs (tournament_id, pair_name, seed)
			VALUES ($1, $2, $3)
		...
		for _, part := range splitPairNames(p.Name) {
			pid, ok, err := resolveTournamentPlayer(ctx, tx, s.schema, part)
			if err != nil {
				return nil, err
			}
			if !ok {
			...
			if _, err := tx.Exec(ctx, `
				INSERT INTO `+s.schema+`.tournament_pair_players (pair_id, player_id) VALUES ($1::uuid, $2::uuid)`,
	...
	for groupID, pairIDs := range snap.Groups {
		for pos, pairID := range pairIDs {
			...
			if _, err := tx.Exec(ctx, `
				INSERT INTO `+s.schema+`.tournament_groups (tournament_id, group_id, pair_id, position)
	...
	for i, m := range snap.Matches {
		...
		var picID *string
		if m.PICName != nil {
			if pid, ok, err := resolveTournamentPlayer(ctx, tx, s.schema, *m.PICName); err != nil {
				return nil, err
			} else if ok {
				picID = &pid
			}
		...
		if _, err := tx.Exec(ctx, `
			INSERT INTO `+s.schema+`.tournament_matches
...
func resolveTournamentPlayer(ctx context.Context, tx pgx.Tx, schema string, name string) (string, bool, error) {
	...
	var pid string
	err := tx.QueryRow(ctx, `SELECT player_id::text FROM `+schema+`.player_aliases WHERE alias_name = $1`, norm).Scan(&pid)
	if err == nil {
		return pid, true, nil
	}
	if !errors.Is(err, pgx.ErrNoRows) {
	...
	pid, err = registerPlayerInTx(ctx, tx, schema, v, v, "")
	if err != nil {
		return "", false, err
	}
	return pid, true, nil
...
func registerPlayerInTx(ctx context.Context, tx pgx.Tx, schema string, name, canonical, gender string) (string, error) {
	...
	if canonNorm != "" && canonNorm != aliasNorm {
		err := tx.QueryRow(ctx, `SELECT player_id::text FROM `+schema+`.player_aliases WHERE alias_name = $1`, canonNorm).Scan(&pid)
	...
	if pid == "" {
		...
		if err := tx.QueryRow(ctx, `
			INSERT INTO `+schema+`.players (canonical_name, gender) VALUES ($1, $2)
		...
		if _, err := tx.Exec(ctx, `
			UPDATE `+schema+`.players SET gender = $2, updated_at = now()
			WHERE id = $1::uuid AND (gender IS NULL OR gender != $2)`, pid, gender); err != nil {
			return "", err
		}
	}
	if _, err := tx.Exec(ctx, `
		INSERT INTO `+schema+`.player_aliases (alias_name, player_id) VALUES ($1, $2::uuid)
		ON CONFLICT (alias_name) DO NOTHING`, aliasNorm, pid); err != nil {
		return "", err
	}
	// Re-query — TOCTOU-safe: kalau ada request lain menang race, pakai id meeka.
	err = tx.QueryRow(ctx, `SELECT player_id::text FROM `+schema+`.player_aliases WHERE alias_name = $1`, aliasNorm).Scan(&pid)
	return pid, err
}
