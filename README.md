...
- **API**: GitHub webhook → `webhook.service` di VPS → `deploy/deploy-vps.sh` → `podman build` lokal → restart quadlet `majadu-api`. Detail: `deploy/webhook.json` + `deploy/deploy-vps.sh`.
